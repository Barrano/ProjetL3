...
$resto=false;
   ...
   if($resto)
    {
    $db=database::connect();
    $statement=$db->prepare('select * from restauration where id_restaurant=?');
    $statement->execute(array($id));
    $item=$statement->fetch();
    $_SESSION['nom_rest']=$item['nom_restaurant'];
    $_SESSION['adresse_rest']=$item['adresse_restaurant'];
    $nom_rest=$_SESSION['nom_rest'];
    $adresse_rest=$_SESSION['adresse_rest'];
    }
    $description=$choix="";
         ...
         while($item=$resu->fetch())
         {
             if($email==$item['email_uti'] and $mdp==$item['password_uti'])
             {
                 $reto="restaurant";
                    ...
                    values('".$id_uti."','".$nom_rest."','".$adresse_rest."','".$date."','".$description."','".$reto."')");
                 
                 $res=$db->query("select id_uti,restaurant_n from utilisateur  where id_uti='".$uticone."' ");
                                   $ligne=$res->fetch();
              ...
              $result=$db->prepare(" UPDATE utilisateur SET restaurant_n=? where id_uti=? ");
                       ...
                       $result->execute(array($resto,$uticone)); 
...
<html>
    ...
    <body>
        <div class="formulaire">
             <form action="verifiResto.php" method="POST"  class="inscription">
              <table>
                 <div class="affiche">
                      <tr>
                        <td><label >nom:</label> </td>
                        <td>
                             <label style="color:#000; font-size:15px; font-family:italic;"><?php echo ' '.$nom_rest; ?></label>
                         </td>
                      </tr> 
                     <tr>
                        <td> <label >adresse:</label> </td>
                        <td>
                            <label style="color:#000; font-size:15px; font-family:italic;" ><?php echo ' '.$adresse_rest; ?></label>
                         </td>
                    </tr>    
                </div>
                <div class="clean"></div>
