...
$resto=false;
   ...
   if($resto)
    {
    $db=database::connect();
    $statement=$db->prepare('select * from boite_nuit where id_boite=?');
    $statement->execute(array($id));
    $item=$statement->fetch();
    $_SESSION['nom_nuit']=$item['nom_boite'];
    $_SESSION['adresse_nuit']=$item['adresse_boite'];
    $nom_nuit=$_SESSION['nom_nuit'];
    $adresse_nuit=$_SESSION['adresse_nuit'];
    }
    $description=$choix="";
        ...
        $db=database::connect();
         ...
         while($item=$resu->fetch())
         {
             ...
             {      $boite="boite";
                    ...
                    values('".$id_uti."','".$nom_nuit."','".$adresse_nuit."','".$date."','".$description."','".$boite."')");
              
              
              $res=$db->query("select id_uti,boite_n from utilisateur  where id_uti='".$uticone."' ");
                                   $ligne=$res->fetch();
                                  $boite=$ligne['boite_n'];
                                  $boite=$boite+1;
              ...
              $result=$db->prepare(" UPDATE utilisateur SET boite_n=? where id_uti=? ");
                       ...
                       $result->execute(array($boite,$uticone)); 
...
<html>
    ...
    <body>
        <div class="formulaire">
             <form action="verifiboite.php" method="POST"  class="inscription">
                 <table>
                 <div class="affiche">
                    <tr>
                        <td>
                       ...
                       <td> 
                            <label style="color:#000; font-size:15px; font-family:italic;"><?php echo ' '.$nom_nuit; ?></label>
                        </td>
                    </tr>
                    <tr>
                        <td>
                       ...
                       <td> 
                             <label  style="color:#000; font-size:15px; font-family:italic;"><?php echo ' '.$adresse_nuit; ?></label>
                        </td>
                    </tr>
                </div>
                <div class="clean"></div>
